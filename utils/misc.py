...
def prep_experiment(args, _):
    """
    Make output directories, setup logging, Tensorboard, snapshot code.
    """
    args.exp_path = os.path.join(args.workdir, args.tag, "exp")
    args.tb_exp_path = os.path.join(args.workdir, args.tag, "tb", str(datetime.now().strftime('%m_%d_%H')))
    ...
    if args.local_rank == 0:
        ...
        open(os.path.join(args.exp_path, args.date_str + '.txt'), 'w').write(
            str(args) + '\n\n')
        writer = SummaryWriter(log_dir=args.tb_exp_path, comment=args.tag)
        return writer
    return None
...
def evaluate_eval(args, net, optimizer, scheduler, val_loss, hist, writer, epoch=0, dataset_name=None, dataset=None, curr_iter=0, save_pth=True):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
    ...
    if save_pth:
        last_snapshot = f"epoch_{epoch}.pth"
